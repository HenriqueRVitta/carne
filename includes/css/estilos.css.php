...
/*ARQUIVO DE ESTILOS*/
	...
	// Defini��o de cores do Sistema na Tabela -> SELECT * FROM styles
	...
	//if (isset($_COOKIE['cook_oco_uid'])) {
		...
		$exec = mysqli_query($conec->con,$qry) or die('ERRO NA TENTATIVA DE RECUPERAR AS INFORMA��ES DO TEMA!<BR>'.$qry);
...
// Cor do rodap� e corpo da p�gina
print "body
{
	position: relative;
	width: 100%;	
...
print "table.topo
{
	...
	if ($row['tm_color_topo'] == "IMG_DEFAULT") {
		...
		// Cabe�alho principal do Projeto juntamente com a LOGO
		
		// Imagem de fundo do cabe�alho
		//print "background-image:url('./bk_header.png');";
		
		// Cor de fundo do cabe�alho padr�o do sistema
		print "background-color: #FFFFFF;";
		print "background-repeat: repeat ;";
	} else {
		
		// Cor de fundo do cabe�alho definida pelo usu�rio
		// #d1ead7 cor de fundo da logo Cliente Cl�nica Sa�de e Harmonia - Ipatinga
		print "background-color:".$row['tm_color_topo'].";";
		//print "background-repeat: repeat ;";
	}
	
	// Cor do Usu�rio logado no topo da p�gina
...
// COR dos bot�es antes	background: #F0F8FF; url('./bg.gif') repeat-x #f0f0f0;
...
/*hack para tratar a camada alfa de imagens png (transpar�ncias)*/

...
class dateOpers {
	...
	function formatDate ($data){ //Retorna sa�da no formado AAAA-MM-DD HH:mm:SS
	...
	function fullDays($data1,$data2){
		...
		if ($sep1['1']<=$sep2['1']){
			$first_is_minor = true;
			//print "<br>O hora de in�cio � menor!<br>";
		} else {
			$first_is_minor = false;
			//print "<br>O hora de in�cio � maior!<br>";
		}
	...
	function diasDomingo($data1,$data2){//Retorna a quantidade de Domingos do per�odo
	...
	//	saida: formato da sa�da da fun��o, se ser� em horas ou segundos
	function tempo_valido($data1,$data2,$hora_ini,$hora_fim,$meio_dia,$sabado,$saida){
		...
		if (empty($data1)|| empty($data2)) {
			...
			if ($data1_aux['3']<$hora_ini){ //ABERTURA COMPARADA A HORA DE IN�CIO
			...
			//	entre as duas datas (tamb�m verifica os feriados permanentes);
			...
			$feriados = mysqli_num_rows($resultado);//Em dias úteis

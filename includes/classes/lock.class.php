	...
	*/
	...
	function setLock ($OCO, $UID, $FORCE_EDIT = 0){ //Constructor
		...
		if ($this->OCO != ""){
			...
			$exec_chk = mysqli_query($chk_lock) or die ("ERRO NA TENTATIVA DE ACESSAR AS INFORMA��ES DE LOCK!<br>".$chk_lock);
			...
			if ($achou != 0) {
				if (($row['lck_uid'] == $this->UID) || ($this->FORCE_EDIT == 1)) {
					...
					print "<br>Essa Ocorr�ncia j� est� em edi��o nesse instante pelo usu�rio <b><font color='red'>".$row_nome['nome']."</font></b>. N�o pode ser editada! <br><a onclick='javascript:history.back()'>Voltar</a><br>";

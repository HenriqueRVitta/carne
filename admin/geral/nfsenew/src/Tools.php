    ...
    public function recepcionarLoteRps($arps, $lote)
    {
        ...
        */
        ...
        foreach ($arps as $rps) {
            ...
            $xml = $this->putPrestadorInRps($rps_);
            $xmlsigned = $this->sign($xml, 'InfRps', 'Id');
            $content .= $xmlsigned;
            $i++;
		...
		for ($i = 1; $i < 99; $i++) {
			...
			$arquivogravacao = $dirarquivoremessa.'/'.'LoteRPS'.date('d').date('m').$seq;
			if(!file_exists($arquivogravacao.".XML")) {
				break;
			}
		}
        file_put_contents($arquivogravacao . '.XML', $content);
        
        $cFile = $arquivogravacao . '.XML';

        $GLOBALS['filedownload'] = $cFile;
	    ...
	    //header('Content-Disposition: filename='. $cFile);
	    //header('Content-Disposition: attachment; filename='.basename($cFile));
	    //readfile($cFile);

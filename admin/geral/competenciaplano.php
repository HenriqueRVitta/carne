...
*/
	...
	if ((isset($_POST['submit'])  && ($_POST['submit'] == TRANS('BT_CAD')))) {	
		...
		if($aviso != TRANS('OK_INSERT')) {
			echo "<script>mensagem('J� existe cadastro com essas informa��es');</script>";
		} 
...
<!--
	function valida(){
		...
		if (ok) var ok = validaForm('idcompet_ini','','Compet�ncia Inicial',1);		
		if (ok) var ok = validaForm('idcompet_fim','','Compet�ncia Fim',1);

...
*/
	...
	print "<BODY onLoad='document.localpagto.idnome.focus();'bgcolor='".BODY_COLOR."'>";
	...
	print "<BR><B><font size=4>"."Administra&ccedil;&atilde;o de Local de Pagamento"."</font></B><BR>";

	print "<FORM name='localpagto' method='POST' action='".$_SERVER['PHP_SELF']."' onSubmit=\"return valida()\">";
	...
	if ((!isset($_GET['action'])) && empty($_POST['submit'])) {
	    ...
	    if(liberamenu('Incluir Local Pagamento')=="N"){
		...
		else
		{
			...
			while ($row = mysql_fetch_array($PAGE->RESULT_SQL))
			{
				...
				if(liberamenu('Excluir Local Pagamento')=="N"){
	...
	if ((isset($_GET['action'])  && ($_GET['action'] == "incluir") )&& empty($_POST['submit'])) {
		...
		print "<BR><b><font size=2 color='blue'>"."Inclus&atilde;o do Local Pagamento"."</b></font><BR>";
	...
	if ((isset($_GET['action']) && $_GET['action']=="alter") && empty($_POST['submit'])) {
		...
		print "<BR><b><font size=2 color='blue'>"."Edi&ccedil;&atilde;o Dados do Local Pagamento"."</b></font><BR>";		
	    ...
	    if(liberamenu('Alterar Local Pagamento')=="N"){
	    	$disabled = " disabled='disabled'";
	    	$clasbutton = " class='buttonDisabled'";
	    }
	...
	if (isset($_GET['action']) && $_GET['action'] == "excluir"){
			...
			// Exclui o localpagto
			$query2 = "DELETE FROM carne_localpagto WHERE id='".$_GET['cod']."'";
	...
	if ((isset($_POST['submit'])  && ($_POST['submit'] == TRANS('BT_CAD')))) {	
		...
		$qryl = "SELECT * FROM carne_localpagto WHERE descricao='".$_POST['nome']."'";
		...
		if (!$erro)
		{
			...
			//$lcnome = strtoupper($_POST['nome']);
			$lcnome = retira_acentos_UTF8($_POST['nome']);
			$lcnome = strtoupper($lcnome);

			$query = "INSERT INTO carne_localpagto (descricao,registro,unidade)".
	...
	if ((isset($_POST['submit'])  && ($_POST['submit'] == TRANS('BT_ALTER')))) {	
		...
		$query2 = "UPDATE carne_localpagto SET descricao='".$lcnome."', unidade=".$_SESSION['s_local']." WHERE id=".$_POST['codigo']." ";		

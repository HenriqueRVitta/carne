...
*/
	...
	print "<TABLE border='0' align='left' ".$cellStyle."  width='100%' bgcolor='".BODY_COLOR."'>";
		...
		if ((isset($_POST['search'])) && !empty($_POST['search'])) {
			...
			// Se informado c�digo de barra ou prontuario
		...
		$pagamentos = mysqli_query($conec->con,$query) or die('ERRO NA EXECUÇÂO DA QUERY DE CONSULTA 1! '.$query);
		
		$registros = mysqli_num_rows($resultado);

		// Se n�o encontrou nenhum registro de pagamento
	...
	if ((!isset($_GET['action'])) && empty($_POST['submit'])) {
		 ...
		 */
			...
			// Se n�o � codigo de barra leva mes em branco
			if ((isset($_POST['search'])) && !empty($_POST['search'])) {
			...
			$resultado = mysqli_query($conec->con,$querylista) or die('ERRO NA EXECUÇÂO DA QUERY DE CONSULTA 1! '.$querylista);		
			...
			print "</td></tr>";
				...
				if ((isset($_POST['search'])) && !empty($_POST['search'])) {
					// Se informado c�digo de barra ou prontuario/nrocarne
					if(is_numeric($_POST['search'])) {

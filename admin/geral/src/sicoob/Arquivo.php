...
class Arquivo {
	...
	public function add_boleto($boleto) {
		...
		$detalhes->setBairro_pagador($boleto['bairro_pagador']);		
		$detalhes->setCidade_pagador($boleto['cidade_pagador']);		
		$detalhes->setUf_pagador($boleto['uf_pagador']);		
	...
	public function get_text() {
		...
		//$numero_sequencial = 2;
		$numero_sequencial = $GLOBALS['numero_sequencial'];
		
		
		if(empty($GLOBALS['cabecalho'])) {
			$GLOBALS['cabecalho'] = $dados;
		}
		
		foreach ($this->getDetalhes() as $detalhe) {
			$detalhe->setNumero_sequencial_registro($numero_sequencial++);
			$linhamontada = $detalhe->montar_linha() . self::QUEBRA_LINHA;
			$dados .= $linhamontada;
			
			$GLOBALS['detalhes'] .= $linhamontada;
			
		}
		
		$GLOBALS['numero_sequencial'] = $numero_sequencial;
		
		//montando rodap�
		$trailler = new Trailler();
		$trailler->setNumero_sequencial_regsitro($numero_sequencial++);
		$this->setTrailler($trailler);
		$rodapemontado = $this->getTrailler()->montar_linha();
		$dados .= $rodapemontado;
		$GLOBALS['rodape'] = $rodapemontado;

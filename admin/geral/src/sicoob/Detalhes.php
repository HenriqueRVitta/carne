...
class Detalhes extends Funcoes implements IFuncoes {
	...
	//002 - 003 - 2 -  N
	private $tipo_inscricao_beneficiario = '01'; // 01 CPF 02 CNPJ
	...
	//093 - 093 - 1 - CONSTANTE
	private $tipo_emissao = 2; // 1-Cooperativa 2-Cliente
	 ...
	 */
	public function getTipo_emissao() {
	 ...
	 * @return the $identificacao_registro
	 */
	public function getTipo_inscricao_beneficiario() {
	...
	public function setValo_cobrado_dia_atraso($valo_cobrado_dia_atraso) {
		...
		if(is_numeric($valo_cobrado_dia_atraso)) {
			...
			$valo_cobrado_dia_atraso = $this->add_zeros($valo_cobrado_dia_atraso, 12);
			//verificando a quantidade de caracteres
			if($this->valid_tamanho_campo($valo_cobrado_dia_atraso, 12)) {
	...
	/**
	 * @param field_type $uf_pagador
	 */
	public function setUf_pagador($uf_pagador) {
			//adiciona brancos na string
		$uf_pagador = $this->montar_branco($uf_pagador, 2, 'right');
		//verifica a quantidade de caracteres
		if($this->valid_tamanho_campo($uf_pagador, 2)) { 
			$this->uf_pagador= $uf_pagador;
		}else{
			throw new Exception('Error - UF do pagador invalido, excedido o tamanho maximo de 2 caracteres.');
		}
	}
	...
	public function setCidade_pagador($cidade_pagador) {
		//	die($endereco_pagador);
			$tamanho = strlen($cidade_pagador);
			if($tamanho > 15) {
				
				$cidade_pagador = $this->resume_texto($cidade_pagador, 15);
				
				$cidade_pagador = $this->montar_branco($cidade_pagador, 15, 'right');
				
				if($this->valid_tamanho_campo($cidade_pagador, 15)) {
					$this->cidade_pagador = $cidade_pagador;
				}else {
					throw new Exception('Error - Cidade do pagador invalido, excedido o tamanho maximo de 15 caracteres.');
				}
			}else {
	...
	public function setBairro_pagador($bairro_pagador) {
		//	die($endereco_pagador);
			$tamanho = strlen($bairro_pagador);
			if($tamanho > 15) {
				
				$bairro_pagador = $this->resume_texto($bairro_pagador, 15);
				
				$bairro_pagador = $this->montar_branco($bairro_pagador, 15, 'right');
				
				if($this->valid_tamanho_campo($bairro_pagador, 15)) {
					$this->bairro_pagador = $bairro_pagador;
				}else {
					throw new Exception('Error - Bairro do pagador invalido, excedido o tamanho maximo de 15 caracteres.');
				}
			}else {
	...
	public function setEndereco_pagador($endereco_pagador) {
		//	die($endereco_pagador);
			...
			if($tamanho > 37) {
				...
				$endereco_pagador = $this->resume_texto($endereco_pagador, 37);
				
				$endereco_pagador = $this->montar_branco($endereco_pagador, 37, 'right');
				
				if($this->valid_tamanho_campo($endereco_pagador, 37)) {
	...
	public function setPrimeira_mensagem($primeira_mensagem) {
		...
		$primeira_mensagem = $this->montar_branco($primeira_mensagem, 40, 'right');
		
		if($this->valid_tamanho_campo($primeira_mensagem, 40)) {
			$this->primeira_mensagem = $primeira_mensagem;
		}else {
			throw new Exception('Error - Primeira mensagem invalida, excedido o tamanho maximo de 40 caracteres.');
		}
	}
	...
	public function setCep($cep) {
		...
		if(is_numeric($cep)) {
			...
			if($this->valid_tamanho_campo($cep, 8)) {
				$this->cep = $this->add_zeros($cep, 8);
			}else {
	...
	public function setCarteira($carteira) {
		...
		if(is_numeric($carteira)) {
			$carteira = $this->add_zeros($carteira, 2);
	...
	public function montar_linha() {
		...
		$linha = 
			...
			// 63 a 74 - 12 Nosso Nro
			$this->getIdentificacao_titulo_banco() .
			$this->getDigito_auto_consferencia_bancaria() .  
			
			// 75 a 76 - 01 - Nro Parcela 01 se parcela unica
			$this->getParcela() .			

			// 77 a 78 - 2 - Grupo de valor
			$this->add_zeros('', 2) .
			...
			// 96 a 101 - 6 - em Branco
			$this->add_zeros('', 6) .

			// 102 a 105 - 4 - em Branco
			$this->montar_branco('', 4) .
			
			// 106 a 106 - 1
			$this->getTipo_emissao() .  // 1-Cooperativa 2-Cliente

			// 107 a 108 - 2 - 01-Simples comRegistro 02-Garantida Caucionada
			$this->getCarteira() .

			// 109 a 110 - 2 - 01 - Entrada de Titulos
			$this->getIdentificacao_ocorrencia() .
			
			// 111 a 120 - 10 - Nro documendo atribuido pela empresa
			$this->getNumero_documento() .  

			// 121 a 126 - 6 - Data Vencto
			$this->getData_vencimento_titulo() .  
			
			// 127 a 139 - 13  - Vlr titulo
			$this->getValor_titulo() .  
			
			// 140 a 142 - 3 - Nro Banco 756
			$this->getBanco_encarregado_cobranca() .
			
			// 143 a 147 - 5 - Preencher com '00000' 
			$this->getAgencia_depositaria() .
			
			// 148 a 149 - 2 - 01-Duplicate
			$this->getEspecie_titulo() .
			
			// 150 a 150 - 1 - 0 - Sem Aceite
			$this->getIdentificacao() .
			
			// 151 a 156 - 6 - Dt Emissao titulo
			...
			// 161 a 172 - 12 - 000000000000 Taxa mora e Taxa multa
			$this->getValo_cobrado_dia_atraso() .  
			
			// 173 a 173 - 1
			$this->getTipo_emissao() .  // 1-Cooperativa 2-Cliente
			
			// 174 a 179 - 6 - em Branco
			$this->add_zeros('', 6) .
			
			// 180 a 192 - 13 - em Branco
			$this->add_zeros('', 13) .
			
			// 193 a 205 - 13 -  0000000000000
			$this->getValor_iof() . 
			
			// 206 a 218 - 2 0000000000000
			$this->getValor_desconto() .  
			
			// 219 a 220 - 2 - 01-CPF 02-CNPJ 
			$this->getIdentificacao_tipo_incricao_pagador() .  
			
			// 221 a 234 - 14 - CPF do Pagador
			$this->getNumero_inscricao_pagador() .  
			
			// 235 a 274 - 40 - Nome Pagador
			$this->getNome_pagador() .  
			
			// 275 a 311 - 37 - Endereco Pagador
			$this->getEndereco_pagador() .  
			
			// 312 a 326 - 15 Bairro Pagador
			$this->getBairro_pagador() .
			
			// 327 a 334 - 8 - Cep Pagador
			$this->getCep() .  
			
			// 335 a 349 - 15 Cidade Pagador
			$this->getCidade_pagador() .
			
			// 350 a 351 - 2 - UF Pagador
			$this->getUf_pagador() .
			
			// 352 a 391 - 40 - Mensagem ao sacador/avalista
			$this->getPrimeira_mensagem() .  
			
			// 392 a 393 - 2 - Nro dias para protesto
			$this->getDiasprotesto() .
			
			// 394 a 395 - 1 - em Branco
			$this->montar_branco('', 1) .

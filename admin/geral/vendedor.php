...
/*      Copyright 2014 MCJ Assessoria Hospitalar e Inform�tica LTDA
		...
		Data: 05/02/2019 17:24 GLPI 12698
...
*/
	...
	print "<BODY onLoad='document.vendedor.idnome.focus();'bgcolor='".BODY_COLOR."'>";
	...
	print "<BR><B><font size=4>"."Administra&ccedil;&atilde;o de Vendedor"."</font></B><BR>";

	print "<FORM name='vendedor' method='POST' action='".$_SERVER['PHP_SELF']."' onSubmit=\"return valida()\">";
	...
	if ((!isset($_GET['action'])) && empty($_POST['submit'])) {
		...
		else
		{
			...
			print "</tr>";
			//------------------------------------------------------------- INICIO ALTERACAO --------------------------------------------------------------
			print "<TR class='header'><td class='line' width='30%'>"."Nome do Vendedor"."</TD>"."<td class='line' width='30%' >"."Nro Celular"."</TD>"."</TD>"."<td class='line' width='30%' >"."Email"."</TD>".
			...
			while ($row = mysqli_fetch_array($PAGE->RESULT_SQL))
			{
				...
				print "<td class='line'>".$row['celular']."</td>";
	...
	if ((isset($_GET['action'])  && ($_GET['action'] == "incluir") )&& empty($_POST['submit'])) {
		...
		print "<BR><b><font size=2 color='blue'>"."Inclus&atilde;o de Vendedor"."</b></font><BR>";
		...
		print "<TD class='line'><a href='vendedor.php'><img height='22' width='22' src='".ICONS_PATH."voltar.png' title='Voltar'></a></TD>";
		print "</TR><TR>";		
		print "<TD width='20%' align='left' bgcolor='".TD_COLOR."'>"."Nome do Vendedor".":</TD>";
		...
		print "<TD width='30%' align='left' bgcolor='".BODY_COLOR."'><INPUT type='text' name='celular' class='text3' maxlength='45' id='idcelular' onkeyup=\"maskIt(this,event,'(##)####-#####')\" value=''></td>";
		print "</TR><TR>";
		
		print "</TR><TR>";		
		print "<TD width='20%' align='left' bgcolor='".TD_COLOR."'>"."Email".":</TD>";
		print "<TD width='20%' align='left' bgcolor='".BODY_COLOR."'><INPUT type='text' class='text' name='email' maxlength='200' id='idemail'></td>";
		print "</TR><TR>";		
		...
		print "<option value='2'>Inativo</option>";
		print "</select>";
		print "</TD></TR>";
	...
	if ((isset($_GET['action']) && $_GET['action']=="alter") && empty($_POST['submit'])) {
		...
		print "</TR><TR>";		
		print "<TD width='20%' align='left' bgcolor='".TD_COLOR."'>"."Email".":</TD>";
		print "<TD width='20%' align='left' bgcolor='".BODY_COLOR."'><INPUT type='text' class='text' name='email' maxlength='200' id='idemail' value='".$row['email']."'></td>";
		print "</TR><TR>";		

		if($row['ativo'] == 1) { $ativo = " selected"; } else { $ativo = ""; }
		if($row['ativo'] == 2) { $inativo = " selected"; } else { $inativo = ""; }
	...
	if (isset($_GET['action']) && $_GET['action'] == "excluir"){
			...
			// Exclui o grupo
			$query2 = "DELETE FROM carne_vendedor WHERE id='".$_GET['cod']."'";
	...
	if ((isset($_POST['submit'])  && ($_POST['submit'] == TRANS('BT_CAD')))) {	
		...
		$qryl = "SELECT * FROM carne_vendedor WHERE nome='".$_POST['nome']."'";
		...
		if (!$erro)
		{
			...
			$lcemail = strtolower($_POST['email']);
			
			$query = "INSERT INTO carne_vendedor (nome,email,celular,ativo,unidade)".
					" values ('".$lcnome."','".$lcemail."','".$_POST['celular']."',".$_POST['situacao'].",".$_SESSION['s_local'].")";
	...
	if ((isset($_POST['submit'])  && ($_POST['submit'] == TRANS('BT_ALTER')))) {	
		...
		$query2 = "UPDATE carne_vendedor SET nome='".$lcnome."', unidade=".$_SESSION['s_local'].", email='".$_POST['email']."', celular='".$_POST['celular']."', ativo = ".$_POST['situacao']."  WHERE id=".$_POST['codigo']." ";		
...
<!--
	function valida(){
		...
		if (ok) var ok = validaForm('idnome','','Nome do Vendedor',1);		

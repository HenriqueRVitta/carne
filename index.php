...
is_file( "./includes/config.inc.php" )
	or die( "VocÃª precisa configurar o arquivo config.inc.php em INCLUDES/para iniciar o uso do !<br>Leia o arquivo <a href='LEIAME.txt'>LEIAME.TXT</a> para obter as principais informaÃ§Ãªs sobre a instalaÃ§a!".
...
print "<table class='barra' border='0' id='barra'><tr>";
	...
	if (empty($_SESSION['s_permissoes'])&& $_SESSION['s_nivel']!=1){
		...
		$execconf = mysql_query($qryconf) or die('NÃ£o foi possÃ­vel ler as informaÃ§Ãµes de configuraÃ§Ã£o do sistema!');
		...
		*/
		...
		$execStyle = mysql_query($qryStyle) or die('ERRO NA TENTATIVA DE RECUPERAR AS INFORMAÃ‡Ã”ES DE ESTILOS!<BR>'.$qryStyle);
...
var GLArray = new Array();
	...
	function valida(){
		...
		var ok = validaForm('idLogin','ALFAFULL','UsuÃ¡rio',1)
